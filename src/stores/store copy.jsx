...
class Store {
  ...
  deployFlashloan1 = (payload) => {
    ...
    this._callDeployFlashLoan1(account, (err, res) => {
      if(err) {
        return emitter.emit(ERROR, err);
      }
  ...
  _callDeployFlashLoan1 = async (account, callback) => {

    const {mainnet: addresses} = require('../addresses')
    const web3 = new Web3(store.getStore('web3context').library.provider);

    const contractArifact = require('../build/contracts/Flashloan.json');
    ...
    const flashloan1Contract = new web3.eth.Contract(contractAbi)
    // console.log("flashloan1Contract", flashloan1Contract)
    const payload = {
      data: bytecode,
      arguments: [addresses.kyber.kyberNetworkProxy,
        addresses.uniswap.router,
        addresses.tokens.weth,
        addresses.tokens.dai,
        addresses.projectToken.FARB,
        account.address ]
    }

    flashloan1Contract.deploy(payload).send({ from: account.address, gasPrice: web3.utils.toWei(await this._getGasPrice(), 'gwei') })
      .on('transactionHash', function(hash){
        console.log(hash)
        callback(null, hash)
      })
