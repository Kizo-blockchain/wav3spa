...
var kickArray = [
  ...
  return (
    ...
    <Card className="App" variant="outlined" style={{backgroundColor:'#1c1f25',width:'40%'}}>
      <div className="note-wrapper" style={{backgroundColor:'#1c1f25'}}>
         
     <div className="note-wrapper" style={{backGroundColor:'#1c1f25'}}>
       {midi && <SVG instrument={midi.instruments} sources={midi.instruments_sources} bpm={bpm}/>}
      </div>


      </div>
      <div >
      <p>Instruments: {instruments && instruments.map((instrument)=>{

        return (<span style={{marginRight:'5px'}}>{instrument},</span >)
      ...
      <div>
       ...
       <button className="play-button" onClick={() => playstop()}>
        {isPlaying ? "Stop" : "Play"}
      </button>
      </div>
